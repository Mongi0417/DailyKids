...
import com.project.dailykids.adapter.SearchKinderAdapter;
...
public class SearchKinderActivity extends AppCompatActivity implements GetJsonObject, View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layout_search_kinder);
    ...
    private void setButtonToSearchKinderByName() {
        ...
        for (int i = 0; i < mList.size(); i++) {
            if (mList.get(i).getKinderName().contains(edtSearchKinder.getText().toString())) {
                kinderInfo = new KinderInfo(mList.get(i).getKinderName(), mList.get(i).getKinderAddress(), mList.get(i).getKinderTel());
                mSearchList.add(kinderInfo);
            }
        }
